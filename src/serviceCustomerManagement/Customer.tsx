...
const CustomerManagement = () => {
  ...
  const handleAddCustomer = async () => {
    if (!newCustomer().email?.includes('@')) {
      alert("Invalid email address. Email must contain '@'.");
      return;
    }

    try {
      const addedCustomer = await addOrUpdateCustomer(newCustomer()); //there is an error here, however the code still works
  ...
  const handleSaveCustomer = async () => {
    ...
    if (!editingCustomer()?.email.includes('@')) {
      alert("Invalid email address. Email must contain '@'.");
      return;
    }
  ...
  return (
    <div class="p-4">
      ...
      <div class="mb-4 flex justify-between">
        ...
        <button onClick={() => navigate('/adminpanel')} class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
          Admin Panel
        </button>
      </div>
      {addingCustomer() && (
      ...
      {filteredCustomers().length === 0 ? (
        <div class="mt-4 p-2 border rounded bg-red-100 text-red-600">
          Customer not found.
        </div>
      ) : (
